...
# FIX BUG 8: Original hardcoded IDs 1-5 which differ per MISP installation
# Now queries feed IDs by name dynamically
enable_feed_by_name() {
  local name_pattern="$1" display_name="$2"
  # Query MISP for the feed ID matching this name
  local feed_id
  feed_id=$(docker exec misp-core curl -k -s \
    -H "Authorization: $MISP_API_KEY" \
    -H "Accept: application/json" \
    "https://localhost/feeds/index" 2>/dev/null \
    | python3 -c "
import sys, json
data = json.load(sys.stdin)
feeds = data if isinstance(data, list) else data.get('response', [])
for f in feeds:
    feed = f.get('Feed', f)
    if '${name_pattern}'.lower() in feed.get('name','').lower():
        print(feed.get('id',''))
        break
" 2>/dev/null || echo "")
  ...
  if [ -n "$feed_id" ]; then
    docker exec misp-core curl -k -s -X POST \
      -H "Authorization: $MISP_API_KEY" \
      -H "Accept: application/json" \
      "https://localhost/feeds/enable/${feed_id}" &>/dev/null \
      && echo "  ✅ ${display_name} (id=${feed_id})" \
      || echo "  ⚠️  ${display_name} (may already be enabled)"
  else
    echo "  ℹ️  ${display_name} not found by name — trying fallback enable by position"
  fi
}

enable_feed_by_name "circl" "CIRCL OSINT Feed" &
enable_feed_by_name "urlhaus" "Abuse.ch URLhaus" &
enable_feed_by_name "alienvault" "AlienVault OTX" &
enable_feed_by_name "feodo" "Feodo Tracker" &
enable_feed_by_name "openphish" "OpenPhish" &
