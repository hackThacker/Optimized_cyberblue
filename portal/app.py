...
class ChangelogManager:
    ...
    def save_changelog(self, force=False):
        """Save changelog to file - buffered to avoid disk write on every entry"""
        now = time.time()
        pending = len(self.changelog["entries"]) - self._last_saved_count
        # Only write to disk if forced, 20+ pending entries, or 10s since last save
        if not force and pending < 20 and (now - self._last_save_time) < 10:
            return
        try:
            with open(self.changelog_file, 'w') as f:
                json.dump(self.changelog, f, indent=2)
            self._last_save_time = now
            self._last_saved_count = len(self.changelog["entries"])
        except Exception as e:
...
class ContainerMonitor:
    ...
    def __init__(self, changelog_manager):
        ...
        # TTL cache for docker ps results (avoids redundant subprocess calls)
        self._status_cache = {}
        self._status_cache_time = 0
        self._cache_ttl = 15  # seconds

    def get_cached_container_status(self):
        """Return cached container status, refreshing only if TTL expired"""
        now = time.time()
        if now - self._status_cache_time < self._cache_ttl and self._status_cache:
            return self._status_cache
        fresh = self._fetch_container_status()
        self._status_cache = fresh
        self._status_cache_time = now
        return fresh

    def get_all_container_status(self):
        """Get detailed status for all containers (uses TTL cache)"""
        return self.get_cached_container_status()
    ...
    def _monitor_loop(self):
        ...
        while self.monitoring:
            try:
                current_status = self._fetch_container_status()
                self._status_cache = current_status
    ...
    def _fetch_container_status(self):
        """Fetch live container status via docker subprocess (use get_all_container_status for cached version)"""
        try:
            result = subprocess.run(
                ['docker', 'ps', '-a', '--format',
    ...
    def get_tool_container_status(self, all_containers=None):
...
def get_container_stats():
    ...
    try:
        ...
        tool_containers = container_monitor.get_tool_container_status(all_containers)
...
def get_dashboard_metrics():
    ...
    try:
        ...
        # System metrics - interval=None uses last cached measurement (non-blocking)
...
def get_agent_info():
    ...
    try:
        ...
        all_containers = container_monitor.get_all_container_status()
        tool_containers = container_monitor.get_tool_container_status(all_containers)
        veloci_status = tool_containers.get('velociraptor', {})
        veloci_container_info = all_containers.get('velociraptor', {})
